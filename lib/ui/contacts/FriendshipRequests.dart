...
class FriendshipRequestsScreen extends StatefulWidget {
  final User user;

  const FriendshipRequestsScreen({Key key, @required this.user}) : super(key: key);

  @override
  _FriendshipRequestsScreenState createState() =>
      _FriendshipRequestsScreenState(user);
}

class _FriendshipRequestsScreenState extends State<FriendshipRequestsScreen> {
  final User user;
  bool showSearchBar = false;
  TextEditingController controller = TextEditingController();
  final fireStoreUtils = FireStoreUtils();
  static Firestore firestore = Firestore.instance;
  _FriendshipRequestsScreenState(this.user);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: new AppBar(
        title: new Text("طلبات صداقة واردة"),
                      ...
                      if (snap.hasData) {
                        ...
                        children: <Widget>[
                                   TextButton(
                                    ...
                                    onPressed: () async {
                                      bool result = await _onContactButtonClickedAccept(
                                          contact, index, false);
                                      if (result) {
                                        hideProgress();
                                        setState(() {});
